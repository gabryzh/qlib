# 基准性能
本页列出了一批为寻找alpha设计的模型。每个模型都试图为每天的所有股票给出分数/预测（例如预测股票未来的超额收益）。模型的分数/预测将被用作挖掘出的alpha。投资于得分较高的股票预计将产生更多利润。

alpha的评估方式有两种。
1. alpha与未来收益之间的相关性。
1. 基于alpha构建投资组合并评估最终的总回报。
   - 指标的解释可以在[这里](https://qlib.readthedocs.io/en/latest/component/report.html#id4)找到

以下是每个基准模型在Qlib的`Alpha360`和`Alpha158`数据集上分别使用中国A股和CSI300数据运行的结果。每个指标的值是基于20次不同随机种子的运行计算出的均值和标准差。

下面显示的数字展示了每个模型整个`workflow`的性能。我们将在不久的将来更新`workflow`以及模型以获得更好的结果。
<!--
> 如果您需要重现以下结果，请使用**v1**数据集：`python scripts/get_data.py qlib_data --target_dir ~/.qlib/qlib_data/cn_data --region cn --version v1`
>
> 在新版本的qlib中，默认数据集是**v2**。由于数据是从YahooFinance API收集的（不是很稳定），*v2*和*v1*的结果可能会有所不同 -->

> 注意:
> 0.8.0版本之后的回测与之前的版本有很大的不同。请查看更新日志以了解差异。

> 注意:
> 我们实现和微调模型的资源非常有限。我们尽了最大努力来公平地比较这些模型。但有些模型的潜力可能比下表中看起来的要大。我们非常欢迎您的贡献来探索它们的潜力。
...
| GRU(Kyunghyun Cho, et al.) | Alpha158(含20个选定特征) | 0.0315±0.00 | 0.2450±0.04 | 0.0428±0.00 | 0.3440±0.03 | 0.0344±0.02 | 0.5160±0.25 | -0.1017±0.02 |
| LSTM(Sepp Hochreiter, et al.) | Alpha158(含20个选定特征) | 0.0318±0.00 | 0.2367±0.04 | 0.0435±0.00 | 0.3389±0.03 | 0.0381±0.03 | 0.5561±0.46 | -0.1207±0.04 |
...
| ALSTM (Yao Qin, et al.) | Alpha158(含20个选定特征) | 0.0362±0.01 | 0.2789±0.06 | 0.0463±0.01 | 0.3661±0.05 | 0.0470±0.03 | 0.6992±0.47 | -0.1072±0.03 |
| GATs (Petar Velickovic, et al.) | Alpha158(含20个选定特征) | 0.0349±0.00 | 0.2511±0.01 | 0.0462±0.00 | 0.3564±0.01 | 0.0497±0.01 | 0.7338±0.19 | -0.0777±0.02 |
| TRA(Hengxu Lin, et al.) | Alpha158(含20个选定特征) | 0.0404±0.00 | 0.3197±0.05 | 0.0490±0.00 | 0.4047±0.04 | 0.0649±0.02 | 1.0091±0.30 | -0.0860±0.02 |
...
| TFT (Bryan Lim, et al.) | Alpha158(含20个选定特征) | 0.0358±0.00 | 0.2160±0.03 | 0.0116±0.01 | 0.0720±0.03 | 0.0847±0.02 | 0.8131±0.19 | -0.1824±0.03 |
...
- 选定的20个特征是基于lightgbm模型的特征重要性。
- DoubleEnsemble的基础模型是LGBM。
- TCTS的基础模型是GRU。
- 关于数据集
  - Alpha158是一个表格数据集。不同特征之间的空间关系较少。每个特征都是由人工精心设计的（即特征工程）
  - Alpha360包含原始价格和成交量数据，没有太多的特征工程。在时间维度上，特征之间有很强的空间关系。
- 指标可分为两类
...
## CSI500上的结果
CSI500上的结果不完整。欢迎为csi500上的模型提交PR！

将CSI300中的先前模型转移到CSI500非常容易。您只需几个命令就可以尝试模型。
```
...
# 或者自动运行多次并获得汇总结果。
cd ../../
python run_all_model.py run 3 lightgbm Alpha158 csi500 # 对于具有随机性的模型，请运行20次。
```
...
如果您想贡献您的新模型，可以按照以下步骤操作。
1. 为您的模型创建一个文件夹
2. 文件夹包含以下项目（您可以参考[此示例](https://github.com/microsoft/qlib/tree/main/examples/benchmarks/TCTS)）。
    - `requirements.txt`：所需的依赖项。
    - `README.md`：对您的模型的简要介绍
    - `workflow_config_<model name>_<dataset>.yaml`：可由`qrun`读取的配置。鼓励您在所有数据集上运行您的模型。
3. 您可以将您的模型集成为[此文件夹](https://github.com/microsoft/qlib/tree/main/qlib/contrib/model)中的一个模块。
4. 请在上面的**基准测试表**中更新您的结果，例如[Alpha360](#alpha158-dataset)、[Alpha158](#alpha158-dataset)（每个指标的值是基于**20次**不同随机种子的运行计算出的均值和标准差。您可以通过Qlib提供的自动化[脚本](https://github.com/microsoft/qlib/blob/main/examples/run_all_model.py)完成上述操作，并在.md文件中获取最终结果。如果您没有足够的计算资源，可以在PR中寻求帮助）。
5. 在[新闻列表](https://github.com/microsoft/qlib#newspaper-whats-new----sparkling_heart)和[模型列表](https://github.com/microsoft/qlib#quant-model-paper-zoo)的索引页面中更新信息。

最后，您可以发送PR进行审查。（[这是一个示例](https://github.com/microsoft/qlib/pull/1040)）
...
答：名称为`*_ts.py`的模型是为`TSDatasetH`设计的（`TSDatasetH`将自动从表格数据创建时间序列）。名称为`*.py`的模型是为`DatasetH`设计的（`DatasetH`通常用于表格数据。但如果列具有时间序列关系，用户仍然可以在表格数据集上应用时间序列模型）。
