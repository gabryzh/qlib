...
"""
注意:
- 此脚本是导入示例数据到 Qlib 的演示
- !!!!!!!!!!!!!!!TODO!!!!!!!!!!!!!!!!!!!:
    - 它的结构设计得不好，非常丑陋，欢迎您的贡献，使导入数据集更容易
"""
...
def add_one_stock_daily_data(filepath, type, exchange_place, arc, date):
    ...
    :param exchange_place: "SZ" 或 "SH"
    :param type: "tick", "orderbook", ...
    :param filepath: csv 的路径
    :param arc: 由进程创建的 arclink
    """
    ...
    if len(error_index_list) > 0:
        print("错误: {}, {}".format(filepath, len(error_index_list)))
    ...
    if symbol in lib.list_symbols():
        print("更新 {0}, date={1}".format(symbol, date))
        if df.empty == True:
            return error_index_list
        lib.update(symbol, df, chunk_size="D")
    else:
        print("写入 {0}, date={1}".format(symbol, date))
...
def add_data(tick_date, doc_type, stock_name_dict):
    ...
    if doc_type not in DOC_TYPE:
        print("doc_type 不在 {} 中".format(DOC_TYPE))
        return
    try:
        ...
        f.write("完成: date:{}, 耗时:{}, 结束时间: {}".format(tick_date, total_time, time.time()) + "\n")
        f.close()
        ...
        for doc_type in doc_type_l:
            ...
            for stock_name in stock_name_list:
                if stock_name not in stock_name_exist:
                    ...
                    pdf.index.name = "date"  # 必须有一个名为 date 的列
                    lib.write(stock_name, pdf)
            print("初始化计数: {}".format(initialize_count))
            print("任务: {}".format(date_list))
            a.reset()
